/**
 * PRCreator — Creates Pull Requests for the AI healing branch
 * 
 * Creates PR from AI_Fix branch → default branch
...
export class PRCreator {
    ...
    _buildPRBody(issues, fixes, retryCount, ciStatus) {
        const statusEmoji = ciStatus === 'PASSED' ? '✅' : '❌';
        const fixedCount = fixes.filter((f) => f.status === 'applied').length;

        let body = `## 🤖 Autonomous AI Fix Report
...
`;
        ...
        body += `\n---\n\n`;
        body += `> 🛡️ Generated by [CodeGuard Pro](https://github.com) Autonomous Healing Agent\n`;
        body += `> ⏱️ ${new Date().toISOString()}\n`;
